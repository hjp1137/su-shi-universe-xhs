...
# -*- coding: utf-8 -*-
"""
更新 data/works.json，补充任务8要求的完整诗词与人生节点内容体验字段
"""
...
def update_works():
    ...
    for w in works:
        wid = w["id"]
        if wid in WORKS_RICH_DATA:
            rich = WORKS_RICH_DATA[wid]
            w["time_label"] = rich["time_label"]
            w["place_label"] = rich["place_label"]
            w["lead_quote"] = rich["lead_quote"]
            w["lead_guide"] = rich["lead_guide"]
            w["life_background"] = rich["life_background"]
            w["original_text"] = rich["original_text"]
            w["why_at_this_moment"] = rich["why_at_this_moment"]
            w["how_it_responds"] = rich["how_it_responds"]
            w["modern_meaning"] = rich["modern_meaning"]
            w["source_note"] = rich["source_note"]
            updated_count += 1

    with open(WORKS_FILE, "w", encoding="utf-8") as f:
        json.dump(works, f, ensure_ascii=False, indent=2)

    print(f"[SUCCESS] 成功更新 {updated_count}/{len(works)} 篇作品的结构化详情字段！")
